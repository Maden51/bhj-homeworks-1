class Autocomplete {
  ...
  getMatches( text ) {
    ...
    const filteredOptions = options.filter(e => e.text.includes(text));
    const inputedOptions = filteredOptions.map(e => {return {'text': e.text, 'value': e.value}});

    return inputedOptions;
  }
}
